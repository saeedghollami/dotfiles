...
function _base_prompt() {
  local base="%{$fg[magenta]%}%n%{$reset_color%}%{$fg[cyan]%}@%{$reset_color%}%{$fg[yellow]%}%m%{$reset_color%}%{$fg[red]%}:%{$reset_color%}"
  
  echo "$base"

}

function _post_prompt() {
  
  local prompt_length="$(pwd|wc -c)"
  local post=""
  local nl=$'\n%{\r%}';

  if [[ $prompt_length -gt 30 ]]; then
